...
public class ContentsResultsController {
    ...
    @GetMapping(path = "/resultId/{resultId}")
    public ResponseEntity<List<ContentsResultsDTO>> getContentsResultsByResultsId(@PathVariable("resultId") final int resultId) throws Exception {
        final List<ContentsResultsDTO> resultsByIdList = contentsResultsService.getContentsResultsByResultId(resultId);
        return ResponseEntity.ok().body(resultsByIdList);
    }
